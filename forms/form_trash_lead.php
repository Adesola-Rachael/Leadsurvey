...
if ( isset($_POST['trash_submit']) && $_POST['trash_submit'] == "trash lead"){   
    ...
    if($trash_error_msg != ""){
       ...
       $data['message'] = $trash_error_msg;  
        ...
        //updating DB details
        $result1 = FALSE;
        $query1 = "UPDATE leads SET status='trashed' WHERE sn ='".$trash_id."' "; 
        $result1 = $conn->query($query1);

        if ($result1){
           $data['success'] = true;
           $data['status'] = "success";
           $data['message'] = "Lead has been deleted successfully.";
        }else{
           $data['success'] = true;
